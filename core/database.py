...
class DatabaseManager:
    ...
    async def execute_batch_insert(self, query: str, values: List[Any]) -> int:
        """Exécute une insertion batch avec liste de valeurs (async)"""
        async with self.get_async_session() as session:
            try:
                # Exécuter directement la requête batch si elle contient déjà VALUES multiples
                if 'VALUES (' in query and len(values) > 0:
                    # C'est une requête batch formatée - on l'exécute directement  
